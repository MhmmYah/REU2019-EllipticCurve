...
void mamul(u32bits final[], u32bits ina[], u32bits inb[], int alen, int blen, u32bits mod[], int modlen){
	...
	for (int i   = 0; i <  finalsize; i ++)
        final[i] = wkspace[10][i];
    //modredu(final, wkspace[10], mod, finalsize, modlen);
}
...
void cleanMult(u32bits result[], u32bits intA[], u32bits intB[], int intASize, int intBSize, u32bits* modulus, int modlen) {
    ...
    for(int i = 0; i < intASize; i++) {
        for(int j = 0; j < intBSize; j++) {
            ...
            //            stored in temp;
            temp[i+j+1] += ((result[i+j]+hold[0] < hold[0])||
                            (result[i+j]+hold[0] < result[i+j])) ? 1 : 0;
            result[i+j] += hold[0];
            temp[i+j+2] += (((result[i+j+1] + hold[1]) < result[i+j+1])
                            || ((result[i+j+1] + hold[1]) < hold[1]))
            ? 1 : 0;
            result[i+j+1] += hold[1];
        }
    }
    for(int i = 0; i < intASize+intBSize; i++) {
        result[i] += carryOut[2];
        carryOut[2] = ((result[i]+temp[i] < result[i]) ||
                       result[i]+temp[i] < temp[i]) ? 1 : 0;
...
void quickmul(u32bits final[], u32bits ina[], u32bits inb[], int alen, int blen, u32bits *modulus, int modlen){
	...
	for(int i = 0;  i< alen; i++){
		...
		for(int j = 0; j < blen; j++){
			hold = (unsigned long)ina[i] * (unsigned long)inb[j];

			temp = final[i+j] + (hold&0xFFFFFFFF);
			carry = temp<final[i+j] || temp<(hold&0xFFFFFFFF);
			final[i+j] = temp;
			
			temp = final[i+j+1] + ((hold>>32)&0xFFFFFFFF) + carry + carry2;
			if(carry || carry2)
				carry2 = !(temp>final[i+j+1] && temp>(hold&0xFFFFFFFF));
			else
				carry2 = temp<final[i+j+1] || temp<(hold&0xFFFFFFFF);
			final[i+j+1] = temp;
		}
